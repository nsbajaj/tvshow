<template>
  <div class="home row mx-2">
    <div class="col-2">
      <ShowsFilter
      @statusFilter="checkboxFilter"></ShowsFilter>
    </div>
    <div class="col-10">
      <h1>Search results: </h1>
      <!-- Shows -->
      <ul class="list-unstyled" >
        <li class="media my-4" v-for="(item, index) in searchResult.shows.data" :key="item.id" :data-index="index">
      ...
      <!-- Shows -->
      <ul class="list-unstyled" >
        <li class="media my-4" v-for="(item, index) in searchResult.people.data" :key="item.person.id" :data-index="index">
          <router-link :to="{ name: 'Person', params: { id: item.person.id }}">
            <img v-if="item.person.image" :src="item.person.image.medium" class="mr-3 img-fluid rounded" :alt="item.person.name">
            <img v-else src="https://lightwidget.com/wp-content/uploads/2018/05/local-file-not-found-295x300.png" class="mr-3" :alt="item.person.name">
          </router-link>
          <div class="media-body">
            <router-link :to="{ name: 'Person', params: { id: item.person.id }}">
